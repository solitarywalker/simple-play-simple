...
local function load(header)
	...
	t.lane.lane.order	= {{1, 2, 3, 4, 5, 6}}

	-- レーンの横幅{{1P}, {2P}}
	t.lane.lane.w		= {{70, 58, 70, 58, 70, 100}}
	t.lane.lane.a		= 96

	-- ノーツの横幅{{1P}, {2P}}
	t.lane.notes.w		= {{70, 54, 70, 54, 70, 96}}
	-- ノーツの縦幅{{1P}, {2P}}
	t.lane.notes.h		= {{40, 40, 40, 40, 40, 40}}
	...
	-- BGA
	t.frame.bga = {
		{x = 0, y = 0, w = 1080, h = 1080, a = 255, brightness = 255, stretch = 1}
	}

	-- グルーブゲージ
	t.frame.gauge.x		= t.lanes.x[1] -138
	t.frame.gauge.y		= t.lanes.y[1] - 146
	t.frame.gauge.h		= 36
	t.frame.gauge.scale	= 48 -- ゲージの粒数
	t.frame.gauge.w		= t.frame.gauge.scale * 12
	t.frame.gauge.num_x	= t.frame.gauge.x
	...
	-- タイミングビジュアライザ
	t.frame.timingvisualizer.w = t.lanes.inside.w[1] - t.lane.lane.w[1][6] - t.lane.line.px
