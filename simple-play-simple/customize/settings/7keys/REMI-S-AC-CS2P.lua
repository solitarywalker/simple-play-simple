...
local function load(header)
	...
	-- レーンの横幅{{1P}, {2P}}
	t.lane.lane.w		= {{68, 52, 68, 52, 68, 52, 68, 120}}
	t.lane.lane.a		= 96

	-- ノーツの横幅{{1P}, {2P}}
	t.lane.notes.w		= {{68, 52, 68, 52, 68, 52, 68, 120}}
	-- ノーツの縦幅{{1P}, {2P}}
	...
	-- BGA
	t.frame.bga = {
		{x = t.lanes.x[1] - 788 - 230, y = 238, w = 788, h = 788, a = 255, brightness = 255, stretch = 1}
	}

	-- グルーブゲージ
	t.frame.gauge.x		= t.lanes.right_x[1] - 11
	t.frame.gauge.y		= 90
	t.frame.gauge.h		= 34
	t.frame.gauge.scale	= 52 -- ゲージの粒数
	t.frame.gauge.w		= t.frame.gauge.scale * -11
	t.frame.gauge.num_x	= t.frame.gauge.x + - 693
	t.frame.gauge.num_y	= t.frame.gauge.y + 14

	-- ソングプログレスバー
	t.frame.songprogress.x		= t.lanes.right_x[1] - 9
	t.frame.songprogress.range	= t.lanes.h[1] - 80
	t.frame.songprogress.y		= t.lanes.y[1] + t.frame.songprogress.range

	-- BPM
	t.frame.bpm.x = t.lanes.x[1] - 185
	t.frame.bpm.y = 32

	-- スコアグラフ
	t.frame.graph.x = t.lanes.x[1] - 221
	t.frame.graph.y = t.lanes.y[1] - 41

	-- スコア
	t.frame.score.x = t.lanes.x[1] - 219
	t.frame.score.y = t.lanes.y[1] + 706

	-- スコアレート
	t.frame.scorerate.x = t.lanes.x[1] - 771
	t.frame.scorerate.y = 45

	-- 自己ベストとのスコア差
	t.frame.diffbest.x = t.lanes.x[1] - 219
	t.frame.diffbest.y = t.lanes.y[1] + 687

	-- ターゲットとのスコア差
	t.frame.difftarget.x = t.lanes.x[1] - 219
	t.frame.difftarget.y = t.lanes.y[1] + 668

	-- ジャッジカウント
	t.frame.judgecount.x = t.lanes.x[1] - 454
	...
	t.frame.songinfo.x = t.lanes.x[1] - 230 - (t.frame.songinfo.w / 2)
