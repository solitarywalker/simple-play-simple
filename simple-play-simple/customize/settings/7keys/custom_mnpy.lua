...
local function load(header)
	...
	t.lane.lane.order	= {{1, 2, 3, 4, 5, 6, 7, 8}}

	-- レーンの横幅{{1P}, {2P}}
	t.lane.lane.w		= {{90, 78, 90, 78, 90, 78, 90, 140}}
	t.lane.lane.a		= 96

	-- ノーツの横幅{{1P}, {2P}}
	t.lane.notes.w		= {{86, 74, 86, 74, 86, 74, 86, 126}}
	-- ノーツの縦幅{{1P}, {2P}}
	...
	-- キービーム
	t.lane.keybeam.h	= 450
	t.lane.keybeam.a	= 168
	...
	-- 判定ライン
	t.lanes.judgeline.h	= 14
	t.lanes.judgeline.a	= 255

	-- ボム表示倍率
	t.lanes.bomb.magnification = 1.2

	--[[ 計算用 消さないでください
		t.lane.lane.x			{{1P}, {2P}} 各レーンのx座標
		t.lane.keybeam.on.x		{{1P}, {2P}} キービームのx座標(キーオン時)
		t.lane.keybeam.off.x	{{1P}, {2P}} キービームのx座標(キーオフ時)
		t.lane.keybeam.on.w		{{1P}, {2P}} キービームの横幅(キーオン時)
		t.lane.keybeam.off.w	{{1P}, {2P}} キービームの横幅(キーオフ時)
	...
	for i1, v1 in pairs(t.lane.lane.order) do
		...
		for i2, v2 in pairs(v1) do
			...
			t.lane.keybeam.on.x[i1][v2]		= x + t.lane.keybeam.variable_key_on
			t.lane.keybeam.off.x[i1][v2]	= x + t.lane.keybeam.variable_key_off
			t.lane.keybeam.on.w[i1][v2]		= t.lane.lane.w[i1][v2] - (t.lane.keybeam.variable_key_on * 2)
			t.lane.keybeam.off.w[i1][v2]	= t.lane.lane.w[i1][v2] - (t.lane.keybeam.variable_key_off * 2)
	...
	for i = 1, #t.lanes.x do
		...
		t.lanes.ghost.y[i]			= t.lanes.judge.y[i] + 130
		
		-- 判定詳細(FAST/SLOW)
		t.lanes.judgedetail.x[i]	= t.lanes.center_x[i]
		t.lanes.judgedetail.y[i]	= t.lanes.judge.y[i] + 80
	end
	------------------------------------------------------------------------
	...
	t.frame.bga = {
		{x = 1500, y = 600, w = 368, h = 368, a = 255, brightness = 255, stretch = 1}
	}
	-- BGA
	--[[
	t.frame.bga = {
		{x = 0, y = -420, w = 1920, h = 1920, a = 255, brightness = 192, stretch = 1}
	}
	--]]
	
	-- グルーブゲージ
	t.frame.gauge.x		= t.lanes.x[1]
	t.frame.gauge.y		= t.lanes.y[1] - 146
	t.frame.gauge.h		= 36
	t.frame.gauge.scale	= 74 -- ゲージの粒数
	t.frame.gauge.w		= t.frame.gauge.scale * 12
	t.frame.gauge.num_x	= t.lanes.x[1] + t.frame.gauge.w - 112
	t.frame.gauge.num_y	= t.frame.gauge.y + 42

	-- ソングプログレスバー
	t.frame.songprogress.x		= t.lanes.x[1] - 42
	...
	t.frame.score.y = t.frame.graph.y + 60

	-- スコアレート
	t.frame.scorerate.x = t.frame.graph.x
	t.frame.scorerate.y = t.frame.graph.y + 40

	-- 自己ベストとのスコア差
	t.frame.diffbest.x = t.frame.graph.x
	t.frame.diffbest.y = t.frame.graph.y + 120

	-- ターゲットとのスコア差
	t.frame.difftarget.x = t.frame.graph.x
	t.frame.difftarget.y = t.frame.graph.y + 100
	...
	-- タイムレフト	
	t.frame.timeleft.x = t.frame.graph.x
	t.frame.timeleft.y = t.frame.graph.y + 280

	-- 現在のランク
	t.frame.rankpace.x = t.frame.graph.x
	t.frame.rankpace.y = t.frame.graph.y + 160

	-- マスコット
	t.frame.mascot.x = t.frame.graph.x + 72
	t.frame.mascot.y = t.frame.graph.y + 3

	-- 動くマスコット
	t.frame.movingmascot.x = t.frame.graph.x + 72
	t.frame.movingmascot.y = t.frame.graph.y + 3

	-- ぽみゅキャラ
	t.frame.pmchara.x = t.frame.graph.x + 72
	t.frame.pmchara.y = t.frame.graph.y + 3
	 -- ぽみゅキャラの表示倍率
	t.frame.pmchara.magnification	= 1

	-- 楽曲情報
	t.frame.songinfo.w = t.frame.gauge.w - 300
	...
	-- BPMグラフ
	t.frame.bpmgraph.w = t.frame.songprogress.range
	t.frame.bpmgraph.h = 50
	t.frame.bpmgraph.x = t.frame.songprogress.x - (t.frame.songprogress.range / 2) + 20
	t.frame.bpmgraph.y = t.frame.songprogress.y - (t.frame.songprogress.range / 2)
