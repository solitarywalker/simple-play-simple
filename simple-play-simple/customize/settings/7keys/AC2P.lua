...
local function load(header)
	...
	-- レーンの横幅{{1P}, {2P}}
	t.lane.lane.w		= {{51, 39, 51, 39, 51, 39, 51, 90}}
	t.lane.lane.a		= 255

	-- ノーツの横幅{{1P}, {2P}}
	t.lane.notes.w		= {{51, 39, 51, 39, 51, 39, 51, 90}}
	-- ノーツの縦幅{{1P}, {2P}}
	t.lane.notes.h		= {{24, 24, 24, 24, 24, 24, 24, 24}}
	...
	for i = 1, #t.lanes.x do
		...
		t.lanes.judge.y[i]			= t.lanes.y[i] + 114
		
		-- ゴーストスコア
		t.lanes.ghost.x[i]			= t.lanes.center_x[i]
		t.lanes.ghost.y[i]			= t.lanes.judge.y[i] + 134
		
		-- 判定詳細(FAST/SLOW)
		t.lanes.judgedetail.x[i]	= t.lanes.center_x[i]
		t.lanes.judgedetail.y[i]	= t.lanes.judge.y[i] + 74
	end
	------------------------------------------------------------------------
	...
	t.frame.gauge.y		= t.lanes.y[1] - 192
	t.frame.gauge.h		= 32
	t.frame.gauge.scale	= 50 -- ゲージの粒数
	t.frame.gauge.w		= t.frame.gauge.scale * 9
	t.frame.gauge.num_x	= t.frame.gauge.x
	t.frame.gauge.num_y	= t.frame.gauge.y + 42

	-- ソングプログレスバー
	t.frame.songprogress.x		= t.lanes.right_x[1] - 12
	t.frame.songprogress.range	= t.lanes.h[1] - 80
	t.frame.songprogress.y		= t.lanes.y[1] + t.frame.songprogress.range

	-- スコアグラフ
	t.frame.graph.x = t.lanes.x[1] - 250
	t.frame.graph.y = 360

	-- BPM
	t.frame.bpm.x = t.frame.graph.x + 10
	t.frame.bpm.y = 10
	...
	-- 楽曲情報
	t.frame.songinfo.w = t.lanes.w[1]
	t.frame.songinfo.h = 24
	t.frame.songinfo.x = t.lanes.center_x[1]
	t.frame.songinfo.y = 10
