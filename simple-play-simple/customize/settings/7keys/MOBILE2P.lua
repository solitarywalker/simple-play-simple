...
local function load(header)
	...
	-- レーンの横幅{{1P}, {2P}}
	t.lane.lane.w		= {{149, 114, 149, 114, 149, 114, 149, 253}}
	t.lane.lane.a		= 96

	-- ノーツの横幅{{1P}, {2P}}
	t.lane.notes.w		= {{129, 94, 129, 94, 129, 94, 129, 233}}
	-- ノーツの縦幅{{1P}, {2P}}
	...
	do -- レーンを中心に配置する
		local tmp = (t.lane.line.px * (#t.lane.lane.order[1] - 1)) + (t.lane.outline.px * 2)
		for i, v in pairs(t.lane.lane.w[1]) do
			tmp = tmp + v
		end
		t.lanes.x[1] = (header.w / 2) - (tmp / 2)
	end
	...
	-- 判定文字表示倍率
	t.lanes.judge.magnification = 1.5
	...
	-- BGA
	t.frame.bga = {
		{x = 0, y = -420, w = 1920, h = 1920, a = 255, brightness = 255, stretch = 1}
	}

	-- グルーブゲージ
	t.frame.gauge.x		= t.lanes.x[1] + 40
	t.frame.gauge.y		= t.lanes.y[1] - 146
	t.frame.gauge.h		= 36
	t.frame.gauge.scale	= 60 -- ゲージの粒数
	t.frame.gauge.w		= t.frame.gauge.scale * 12
	t.frame.gauge.num_x	= t.frame.gauge.x + t.frame.gauge.w + 6
	t.frame.gauge.num_y	= t.frame.gauge.y

	-- ソングプログレスバー
	t.frame.songprogress.x		= t.lanes.right_x[1] - 22
	t.frame.songprogress.range	= t.lanes.h[1] - 80
	t.frame.songprogress.y		= t.lanes.y[1] + t.frame.songprogress.range

	-- BPM
	t.frame.bpm.x = t.frame.gauge.x
	t.frame.bpm.y = t.frame.gauge.y - 56

	-- スコアグラフ
	t.frame.graph.x = t.lanes.x[1] - 230
	t.frame.graph.y = 360

	-- スコア
	t.frame.score.x = t.frame.graph.x + 78
	t.frame.score.y = t.frame.graph.y + 60

	-- スコアレート
	t.frame.scorerate.x = t.frame.graph.x +78
	t.frame.scorerate.y = t.frame.graph.y + 40

	-- 自己ベストとのスコア差
	t.frame.diffbest.x = t.frame.graph.x + 78
	t.frame.diffbest.y = t.frame.graph.y + 120

	-- ターゲットとのスコア差
	t.frame.difftarget.x = t.frame.graph.x + 78
	...
	-- タイムレフト	
	t.frame.timeleft.x = t.frame.graph.x + 78
	t.frame.timeleft.y = t.frame.graph.y + 280

	-- 現在のランク
	t.frame.rankpace.x = t.frame.graph.x + 78
	t.frame.rankpace.y = t.frame.graph.y + 160

	-- マスコット
	t.frame.mascot.x = t.frame.graph.x
	t.frame.mascot.y = t.frame.graph.y + 3

	-- 動くマスコット
	t.frame.movingmascot.x = t.frame.graph.x
	t.frame.movingmascot.y = t.frame.graph.y + 3

	-- ぽみゅキャラ
	t.frame.pmchara.x = t.frame.graph.x
	...
	t.frame.songinfo.x = t.frame.gauge.x + (t.frame.songinfo.w / 2) + 200
