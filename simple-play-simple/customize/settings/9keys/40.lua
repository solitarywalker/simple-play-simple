...
local function load(header)
	...
	t.lane.lane.order	= {{1, 2, 3, 4, 5, 6, 7, 8, 9}}

	-- レーンの横幅{{1P}, {2P}}
	t.lane.lane.w		= {{90, 78, 90, 78, 90, 78, 90, 78, 90}}
	t.lane.lane.a		= 96

	-- ノーツの横幅{{1P}, {2P}}
	t.lane.notes.w		= {{80, 64, 80, 64, 80, 64, 80, 64, 80}}
	-- ノーツの縦幅{{1P}, {2P}}
	t.lane.notes.h		= {{40, 40, 40, 40, 40, 40, 40, 40, 40}}
	...
	for i = 1, #t.lanes.x do
		...
		t.lanes.ghost.y[i]			= t.lanes.judge.y[i] + 240
		
		-- 判定詳細(FAST/SLOW)
		t.lanes.judgedetail.x[i]	= t.lanes.center_x[i]
		t.lanes.judgedetail.y[i]	= t.lanes.judge.y[i] + 160
	end
	------------------------------------------------------------------------
	...
	-- BGA
	t.frame.bga = {
		{x = 26, y = 284, w = 512, h = 512, a = 255, brightness = 255, stretch = 1}
	}
	...
	-- タイミングビジュアライザ
	t.frame.timingvisualizer.w = t.lanes.inside.w[1]
	t.frame.timingvisualizer.h = 12
	t.frame.timingvisualizer.x = t.lanes.inside.x[1]
